...
echo "Qt, Dolphin, etc. will NOT be removed."
echo
...
#
# Request.Close is intentionally NOT used here.
#
# org.freedesktop.impl.portal.Request belongs to individual
# portal request objects and is not exported on the backend's
# main object path.
#
# We terminate the actual backend process instead.
# ============================================================
...
if [[ -n "${KFILE_PID}" ]]; then
    ...
    # Give the backend a short amount of time to shut down cleanly.
    for _ in {1..20}; do
    ...
    # If it is still alive, use SIGKILL as a final fallback.
    if kill -0 "${KFILE_PID}" 2>/dev/null; then
...
for line in lines:
    ...
    # Remove only our kfile FileChooser entry.
    if (
...
# Remove the directory if it became empty.
...
echo
echo "Checking whether the kfile D-Bus service is still present..."

if busctl --user status "${BUS_NAME}" >/dev/null 2>&1; then
    echo
    echo "Warning: ${BUS_NAME} is still running."
    echo "The process may exit shortly."
else
    echo "kfile D-Bus service is no longer running."
fi
...
echo "  ${DBUS_SERVICE}"
echo
echo "The kfile FileChooser entry was removed from:"
echo "  ${USER_CONFIG}"
echo
echo "System packages were NOT removed."
echo
echo "If you want to use another portal backend, configure it in:"
echo
echo "  ${USER_CONFIG}"
echo
echo "For example, GTK can be selected with:"
echo
echo "  org.freedesktop.impl.portal.FileChooser=gtk;"
echo
